...
public class bl_RoomController : bl_PhotonHelper
{
    ...
    static public bool NoResume = false;
    ...
    void Inputs()
    {
        ...
        if (Input.GetKeyDown(PlayerListKey) && bl_GameController.isPlaying && !m_ShowPlayerList)
        {
            ...
            if (m_ListManager != null) { m_ListManager.ShowPlayerList(true); }
        }
        if (Input.GetKeyUp(PlayerListKey) && bl_GameController.isPlaying && m_ShowPlayerList)
        {
            ...
            if (m_ListManager != null) { m_ListManager.ShowPlayerList(false); }
        }
        #endregion
    }
    /// <summary>
    ...
    public void UpdatePlayerList()
    {
        ...
        for (int i = 0; i < players.Length; i++)
        {
            ...
            r.GetComponent<bl_PlayerList>().GetInfo(players[i].name, pi.ToString(), players[i]);
            r.transform.SetParent(PlayerListPanel, false);
        }
    ...
    static public void ChangeResumeBool()
    {
        if(NoResume == false)
        { 
            Pause = !Pause;
