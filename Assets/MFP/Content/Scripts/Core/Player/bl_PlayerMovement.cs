...
public class bl_PlayerMovement : bl_PhotonHelper {
    ...
    [SerializeField]private Transform BobObject;
        ...
        else
        {
            ...
            if (grounded)
            {
                ...
                else
                {
                    ...
                    {
                        Vector3 nextBobPos = m_HeadBob.DoHeadBob(controller.velocity.magnitude + (speed * ((m_PlayerState == PlayerState.Stand) ? 1f : 0.7f)));
    ...
    IEnumerator FootStepUpdate()
    {
        while (true)
        {
            ...
            if (controller.isGrounded && vel > 0.2f)
            {
